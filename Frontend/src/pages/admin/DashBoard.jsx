import { ChartLineIcon, CircleDollarSignIcon, icons, PlayCircleIcon, StarIcon } from 'lucide-react';
            ...
            <div className='flex flex-wrap gap-4 w-full'>
                {
                    dashboardCards.map((card, index) => (
                        <div key={index} className='flex items-center justify-between px-4 py-3 bg-primary/10 border border-primary/20 rounded-md max-w-70 w-full'>
                            <div className=''>
            ...
            <BlurCircle top='100px' left='-10%'/>
            {
                dashBoardData.activeShows.map((show) => (
                    <div className='w-55 rounded-lg overflow-hidden h-full pb-3 bg-primary/10 border border-primary/20 hover:-translate-y-1 transition duration-300'>
                        ...
                        <p className='px-2 pt-2 text-sm text-gray-500'>{isoTimeFormat(show.showDateTime)}</p>
                    </div>
                ))
            }
        </div>
